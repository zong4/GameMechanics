...
public class PuzzleGenerator1 : MonoBehaviour
{
            ...
            for (var j = 0; j < gridSize; j++)
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    foreach (var direction in directions)
                    {
                        ...
                        if (IsNotVisited(next, visited) && IsConnected(current, next))
                        {
                            queue.Enqueue(next);
    ...
    private bool IsConnected(Vector2Int a, Vector2Int b)
    {
        ...
        for (var i = 0; i < _solutionPath.Count - 1; i++)
        {
            var p1 = _solutionPath[i];
            var p2 = _solutionPath[i + 1];
    ...
    private void RenderPuzzle()
    {
        ...
        foreach (var cell in _solutionRegions[0]) // First region is always white
        {
            var cellPosition = new Vector3(cell.x, cell.y, 0) - centerOffset;
        ...
        for (var i = 1; i < _solutionRegions.Count; i++)
        {
            ...
            {
                foreach (var cell in _solutionRegions[i])
                {
                    var cellPosition = new Vector3(cell.x, cell.y, 0) - centerOffset;
