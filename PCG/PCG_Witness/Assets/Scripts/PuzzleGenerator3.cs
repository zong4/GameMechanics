        ...
        for (var i = 0; i < gridSize; i++)
        {
            for (var j = 0; j < gridSize; j++)
            {
                if(_whiteCells.Contains(new Vector2Int(i, j)))
                    Instantiate(whiteCellPrefab, new Vector3(i, j, 0) - centerOffset, 
                        Quaternion.identity, transform).transform.localScale = Vector3.one * cellSize;
                else if(_blackCells.Contains(new Vector2Int(i, j)))
                    Instantiate(blackCellPrefab, new Vector3(i, j, 0) - centerOffset, 
                        Quaternion.identity, transform).transform.localScale = Vector3.one * cellSize;
                else
                    Instantiate(cellPrefab, new Vector3(i, j, 0) - centerOffset, 
                        Quaternion.identity, transform).transform.localScale = Vector3.one * cellSize;
            }
        }
